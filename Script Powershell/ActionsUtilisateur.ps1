...
function deleteuser()
{
    ...
    if ( $confirm = "O" )
    {
        ...
        if (($((Get-LocalUser -Name $user).Enabled )) 2> $null)
...
function deactivateuser()
{
    ...
    # Vérification existence du nom du compte 
    While ( ! ( Get-LocalUser -Name $user ) 2> $null )
    {
        Write-Host "`n"
        Write-Host "L'utilisateur $user est introuvable "
        Get-LocalUser -Name $user*
...
function ajouterUtilisateurGroupeLocal
{}

# Sortie d’un groupe local
function supprimerUtilisateurGroupeLocal
{}