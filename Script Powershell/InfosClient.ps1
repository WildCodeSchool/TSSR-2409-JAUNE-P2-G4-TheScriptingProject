...
#    Requêtes d'Informations concernant un Client   #
#                                                   #
#####################################################
...
function disk4
{
    ...
    $size=(Get-ChildItem -force $directory -Recurse -ErrorAction SilentlyContinue| measure Length -Sum).sum
    if ($size -le 0) {
        # Message erreur
        ...
        # format en KB
        $size=((gci -force $directory -Recurse -ErrorAction SilentlyContinue| measure Length -Sum).sum / 1Kb)
        ...
        # format en MB
        $size=((gci -force $directory -Recurse -ErrorAction SilentlyContinue| measure Length -Sum).sum / 1Mb)
        ...
        # format en GB
        $size=((gci -force $directory -Recurse -ErrorAction SilentlyContinue| measure Length -Sum).sum / 1Gb)
